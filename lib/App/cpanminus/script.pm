...
sub parse_options {
    ...
    Getopt::Long::GetOptions(
        ...
        'minicpan=s' => sub { unshift @{$self->{mirrors}}, "file://" . $self->maybe_abs($_[1]); $self->{minicpan} = $_[1] },
        'prompt!'   => \$self->{prompt},
...
sub search_module {
    ...
    if ($self->{minicpan}) {
        $self->chat("Searching $module on minicpan($self->{minicpan}) ...\n");
        my $path = File::Spec->catfile($self->{minicpan}, 'modules', '02packages.details.txt.gz');
        eval "require IO::Zlib; 1" or die "IO::Zlib is required for minicpan support";
        my $fh = IO::Zlib->new($path, 'rb');
        while (<$fh>) { # skip headers
            last unless /\S/;
        }
        while (<$fh>) {
            if (/^\Q$module\E\s+\S+\s+(\S+)$/) {
...
Options:
  ...
  --minicpan                Specify the local minicpan the mirror (e.g. /path/to/minicpan)
...
Examples:
  ...
  cpanm --minicpan /path/to/minicpan DBI                    # use the local minicpan mirror
